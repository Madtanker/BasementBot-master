/**
 *Original by Yemasthui Copyright 2014
...
(function () {
    ...
    var loadChat = function (cb) {
        ...
        $.get("https://rawgit.com/Madtanker/BasementBot/master/lang/langIndex.json", function (json) {
    ...
    var basicBot = {
        ...
        name: "BasementCouch",
        loggedInID: null,
        scriptLink: "https://rawgit.com/Madtanker/BasementBot/master/basicBot.js",
        cmdLink: "http://git.io/pUGW",
        ...
        settings: {
            ...
            welcome: true,
            opLink: "http://www.theedmbasement.com/#!bannedsongs/c17yd",
            ...
            blacklists: {
                ...
                OP: "https://rawgit.com/Madtanker/BasementBot/master/blacklists/Banlist.json"
            }
        },
        room: {
            users: [],
        ...
        userUtilities: {
            ...
            getPermission: function (obj) { //1 requests
                ...
                for (var i = 0; i < botCreatorIDs.length; i++) {
                    if (botCreatorIDs[i].indexOf(u.id) > -1) return 3;
                }
            ...
            dclookup: function (id) {
                ...
                if (pos == 0) return (subChat(basicBot.chat.wasdjing, {name: name}));
                var timeDc = Date.now() - dc;
                var validDC = false;
                if (basicBot.settings.newMaximumDc * 60 * 1000 > timeDc) {
                    validDC = true;
                }
                var time = basicBot.roomUtilities.msToStr(timeDc);
                if (!validDC) return (subChat(basicBot.chat.toolongago, {name: name, time: time}));
            ...
            intervalMessage: function () {
                ...
                if ((basicBot.room.roomstats.songCount % interval) === 0 && basicBot.status) {
                    ...
                    if (basicBot.settings.motdEnabled) {
                        ...
                        if (basicBot.settings.intervalMessagesNew.length === 0) return void (0);
                        var messageNumber = basicBot.room.roomstats.songCount % basicBot.settings.intervalMessagesNew.length;
                        msg = basicBot.settings.intervalMessagesNew[messageNumber];
                    }
                    //API.sendChat('/me ' + msg);
                }
            },
        ...
        eventVoteupdate: function (obj) {
            ...
            for (var i = 0; i < basicBot.room.users.length; i++) {
                ...
                if (basicBot.userUtilities.getUser(basicBot.room.users[i]).vote === -1) {
                    if(basicBot.room.users[i].id != 5951922 && 
                        basicBot.room.users[i].id != 5951933 && 
                        basicBot.room.users[i].id != 5951994 && 
                        basicBot.room.users[i].id != 6381472 && 
                        basicBot.room.users[i].id != 6381478 && 
                        basicBot.room.users[i].id != 6381630) {
                        totalMehs++;
                    }
                }
            }
            ...
            if (basicBot.settings.voteSkipNew) {
                if (totalMehs >= basicBot.settings.voteSkipLimit) {
                    API.sendChat(subChat(basicBot.chat.voteskipexceededlimit, {name: dj.username, limit: basicBot.settings.voteSkipLimit}));
                    API.moderateForceSkip();
                }
            }
            //console.log(totalMehs + " , " + basicBot.settings.voteSkipNew + " , " + basicBot.settings.voteSkipLimit);
        ...
        eventDjadvance: function (obj) {
            ...
            for (var i = 0; i < basicBot.room.historyList.length; i++) {
                if (basicBot.room.historyList[i][0] === obj.media.cid) {
                   ...
                   // API.sendChat(subChat(basicBot.chat.songknown, {plays: plays, timetotal: basicBot.roomUtilities.msToStr(Date.now() - firstPlayed), lasttime: basicBot.roomUtilities.msToStr(Date.now() - lastPlayed)}));
                    timeSinceLast = new Date().getTime() - lastPlayed;
                    if(basicBot.settings.skipifinhistory && timeSinceLast < (3 * 60 * 60 * 1000) && timeSinceLast > 5000) { //If a song was played within the last 3 hours (more than 5 seconds ago)
                        playedRecently = true;
                    }
                    else {
                        basicBot.room.historyList[i].push(new Date().getTime());
                    }
                    alreadyPlayed = true;
                }
            }
            if (playedRecently){
                ...
                msec -= ss * 1000;
                API.sendChat(subChat(basicBot.chat.songinhistory, {name: name, time:(hh + "h" + mm + "m" + ss + "s")}));
                API.moderateForceSkip();
            }
            if (!alreadyPlayed) {
                basicBot.room.historyList.push([obj.media.cid, new Date().getTime()]);
            }
            var newMedia = obj.media;
            if (basicBot.settings.timeGuard && newMedia.duration > basicBot.settings.maximumSongLength * 60 && !basicBot.room.roomevent && API.getDJ().role==0) {
                var name = obj.dj.username;
        ...
        startup: function () {
            ...
            if (basicBot.userUtilities.getPermission(u) < 3) return API.chatLog(basicBot.chat.greyuser);
            basicBot.connectAPI();
            ...
            voteskipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!basicBot.settings.voteSkipNew) basicBot.settings.voteSkipNew = !basicBot.settings.voteSkipNew;
                        if (isNaN(argument)) {
            ...
            togglevoteskipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        basicBot.settings.voteSkipNew = !basicBot.settings.voteSkipNew;
                        if (!basicBot.settings.voteSkipNew) {
            ...
            statusCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        msg += basicBot.chat.skipifinhistory + ': ';
                        if (basicBot.settings.skipifinhistory) msg += 'ON';
                        else msg += 'OFF';
                        msg += '. ';
                        ...
                        if (basicBot.settings.voteSkipNew) msg += 'ON';
                        else msg += 'OFF';
                        msg += '. ';
            ...
            skipHistoryCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (basicBot.settings.skipifinhistory) {
                            basicBot.settings.skipifinhistory = !basicBot.settings.skipifinhistory ;
                            return API.sendChat(subChat(basicBot.chat.toggleoff, {name: chat.un, 'function': basicBot.chat.skipifinhistory}));
                        }
                        else {
                            basicBot.settings.skipifinhistory = !basicBot.settings.skipifinhistory;
                            return API.sendChat(subChat(basicBot.chat.toggleon, {name: chat.un, 'function': basicBot.chat.skipifinhistory}));
                        }
